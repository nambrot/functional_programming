...
trait GameDef {
  ...
  sealed abstract class Move {
    def move(block: Block): Block;
  }
  case object Left  extends Move {
    def move(block: Block) = block.left
  }
  case object Right extends Move {
    def move(block: Block) = block.right
  }
  case object Up    extends Move {
    def move(block: Block) = block.up
  }
  case object Down  extends Move {
    def move(block: Block) = block.down
  }
  ...
  case class Block(b1: Pos, b2: Pos) {
    override def toString = s"B($b1,$b2)"
     ...
     */
    def neighbors: List[(Block, Move)] = List(Left, Up, Down, Right) map (move => (move.move(this), move))
